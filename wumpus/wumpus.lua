...
dodecahedron_graph = {
  { 2, 5,20}, { 1, 3,18}, { 2, 4,16}, { 3, 5,14}, { 1, 4, 6},
  { 5, 7,13}, { 6, 8,20}, { 7, 9,12}, { 8,10,19}, { 9,11,17},
  {10,12,15}, { 8,11,13}, { 6,12,14}, { 4,13,15}, {11,14,16},
  { 3,15,17}, {10,16,18}, { 2,17,19}, { 9,18,20}, { 1, 7,19}
}
...
function rand_shuffle( old_list )
  local list = {}
...
function get_user_input_tokens()
  io.write("> ")
  local str = io.read("*l")
  local t = {}
...
function is_in_room( room_type, number )
...
function describe_room( game, room_index )
  local v
  ...
  for _, v in ipairs(room_exits) do
    if draw_comma then io.write(", ")
    else draw_comma = true end
    io.write(v)
  end
  io.write(".\n")
end

function room_has_exit( room, exit )
...
function new_room_for_wumpus( game )
...
function new_room_for_player( game )
...
function move_player_to_room( game, move_to )
...
function game_move_command( game, userline )
...
function game_shoot_command( game, userline )
...
function game_quit_command()
  return false
end

function game_cheat_command( game )
...
function find_command( com_name )
...
function run_main_state_machine()
  local state = "init"
